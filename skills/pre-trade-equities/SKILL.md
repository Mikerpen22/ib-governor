...
- order type: market / limit / stop / stop-limit (default market); for limit/stop, the price(s)

## Step 1 — Deterministic gate (the facts)
Run the gate's READ-ONLY analysis. It places NO order; it stages a single-use, ~5-minute token:
...
Combine the deterministic facts (Step 1) with any vault judgment (Steps 2–3). You may **escalate** the gate's verdict (toward caution) but never **downgrade** a deterministic BLOCK:
...
Report the result. If `dry_run: true`, the gate is in SAFE mode — the order was logged, NOT sent (this is the shipped default). Never submit without an explicit confirm. The token is single-use and expires (~5 min); if expired, re-run Step 1.
